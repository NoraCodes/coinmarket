...
#if defined(NDEBUG)
# error "Investcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x712824936282e3de68d5826b124087f0cea5a0a42e4776fd69962a36e1df883f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Investcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Investcoin Signed Message:\n";
...
{
    // Investcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2618000000); // Investcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.4 * 24 * 60 * 60; // Investcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Investcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Investcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Investcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0ff28a8503b5b6fae11a58c0bb231faf4822680fcb4647c5adae195e401c30ef");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Times 01/Aug/2018 Investcoin started";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cc9e9502ff480ba0c48e7acf2188cbaea1e734b442e5b4e6de36953b0805ee81b6bf5e48e67df32295fd72610ba3e90319ac4ce38b0f2c017323c933c923432b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533116470;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087307571;

        if (fTestNet)
        {
            block.nTime    = 1533116468;
            block.nNonce   = 386645741;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc15be63ff73796ea65059516341d5a88ebc4b2ecc1540bde904ac7bd613c4f84"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Investcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InvestcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InvestcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InvestcoinMiner(CWallet *pwallet)
{
    printf("InvestcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("investcoin-miner");
    ...
    try { loop {
        ...
        printf("Running InvestcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InvestcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InvestcoinMiner, pwallet));
}
